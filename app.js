(function () {
  ...
  function init(){
    ...
    loginForm.addEventListener('submit',(e)=>{
      ...
      applyAuth(); render(); toast('Login efetuado.');
    });
    ...
    btnCSV.addEventListener('click', ()=>{
      const rows=load(); const headers=['date','start_time','end_time','hall','apartment','resident_name','cups','forks','knives','spoons','plates','guests_text'];
    ...
    fileImport.addEventListener('change', async(e)=>{
      ...
      try{ const txt=await f.text(); const data=JSON.parse(txt); if(!Array.isArray(data)) throw new Error('JSON inválido');
        localStorage.setItem(CONFIG.storageKey, JSON.stringify(data)); render(); toast('Importação concluída.');
      }catch(err){ toast('Falha ao importar: '+err.message); } finally { fileImport.value=''; }
    });
    btnLogout.addEventListener('click',()=>{clearUser();applyAuth();});
  ...
  function openEdit(id){ const rows=load(); const p=rows.find(x=>x.id===id); if(!p)return; state.editingId=id; dialogTitle.textContent='Editar Festa'; fill(p); dialog.showModal(); }
  ...
  function saveParty(){
    ...
    if(state.editingId){ const i=rows.findIndex(x=>x.id===state.editingId); if(i>=0) rows[i]=p; }
    else { rows.push(p); }
